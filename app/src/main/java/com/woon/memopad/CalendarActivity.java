...
public class CalendarActivity extends AppCompatActivity implements EasyPermissions.PermissionCallbacks {
    ...
    // Google Calendar API에 접근하기 위해 사용되는 API 서비스 객체
    ...
    Button addCalendar = (Button) findViewById(R.id.button_main_add_calendar);
    Button addEvent = (Button) findViewById(R.id.button_main_add_event);
    Button getEvent = (Button) findViewById(R.id.button_main_get_event);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Oauth 2.0를 사용하여 구글 계정 및 인증하기 위한 준비
        mCredential = GoogleAccountCredential.usingOAuth2(
                getApplicationContext(),
                Arrays.asList(SCOPES)
        ).setBackOff(new ExponentialBackOff());     // I/O 예외 상황을 대비하여 백오프 정책 사용
    }
    ...
    private String getResultsFromApi() {
        if(!isGooglePlayServicesAvailable()) {                      // Google Play Services를 사용할 수 없는 경우
            ...
            mStatusText.setText("인터넷이 연결되어 있지 않습니다");
        } else {
            // Google Calenar API 호출
            new MakeRequestTask(this, mCredential).execute();
        }
        return null;
    }

    // 기기에 최신 버전의 Google Play Services가 설치되어 있는지 확인
    ...
    // Google Play Services 최신버전 업데이트
    ...
    /**
     * Google Calendar API의 자격 증명(credentials)에 사용할 구글 계정 설정
     *
    ...
    private void chooseAccount() {
        ...
        if (EasyPermissions.hasPermissions(this, Manifest.permission.GET_ACCOUNTS)) {
            ...
            // 사용자에게 GET_ACCOUNTS 권한을 요구하는 다이얼로그를 보여준다.(주소록 권한 요청)
            EasyPermissions.requestPermissions(
                    (Activity) this,
                    "구글 계정을 등록해야 합니다. (via Contacts)",
    ...
    /**
     * 구글 플레이 서비스 업데이트 다이얼로그, 구글 계정 선택 다이얼로그, 인증 다이얼로글에서
     * 되돌아올 때 호출
     * **/
    @Override
    ...
    private class MakeRequestTask extends AsyncTask<Void, Void, String> {
        ...
        /**
         * 백그라운드에서 Google Calendar API 호출 정리
         * */
        @Override
